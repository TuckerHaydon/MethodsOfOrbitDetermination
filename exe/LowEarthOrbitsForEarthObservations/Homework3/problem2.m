%% Preamble
clc; clear all; close all;

%% Given parameters
gravitational_parameter = 3.986004415e14 * (Units.meters)^2 / (Units.seconds)^2;

%% Load the data
file_name = "1d.180s.ECI.txt";
T = readtable(file_name, ...
    "FileType", "delimitedtext", ...
    "Delimiter", " ", ...
    "LeadingDelimitersRule", "ignore", ...
    "ConsecutiveDelimitersRule", "join", ...
    "NumHeaderLines", 6);

time                 = table2array(T(:, 1)); % seconds
time_utc_mjd         = table2array(T(:, 2)); % Julian days
position_history_eci = transpose(table2array(T(:, 3:5))); % meters
velocity_history_eci = transpose(table2array(T(:, 6:8))); % meters per second


%% Load the orbital element check data
file_name = "1d.180s.OEL.txt";
T = readtable(file_name, ...
    "FileType", "delimitedtext", ...
    "Delimiter", " ", ...
    "LeadingDelimitersRule", "ignore", ...
    "ConsecutiveDelimitersRule", "join", ...
    "NumHeaderLines", 11);

OEL.time                         = table2array(T(:, 1)); % seconds
OEL.time_utc_mjd                 = table2array(T(:, 2)); % Julian days
OEL.semimajor_axis               = table2array(T(:, 3)); % meters
OEL.eccentricity                 = table2array(T(:, 4)); % unitless
OEL.inclination                  = deg2rad(table2array(T(:, 5))); % radians
OEL.argument_of_periapsis        = deg2rad(table2array(T(:, 6))); % radians
OEL.longitude_of_ascending_node  = deg2rad(table2array(T(:, 7))); % radians
OEL.mean_anomaly                 = deg2rad(table2array(T(:, 8))); % radians
OEL.argument_of_latitude         = deg2rad(table2array(T(:, 9))); % radians

%% Load the revolutions check data
file_name = "1d.180s.REV.txt";
T = readtable(file_name, ...
    "FileType", "delimitedtext", ...
    "Delimiter", " ", ...
    "LeadingDelimitersRule", "ignore", ...
    "ConsecutiveDelimitersRule", "join", ...
    "NumHeaderLines", 11);

REV.num_data_points              = table2array(T(:, 1)); % unitless
REV.start_time                   = table2array(T(:, 2)); % Julian days
REV.end_time                     = table2array(T(:, 3)); % meters
REV.UNKNOWN                      = table2array(T(:, 4)); % meters
REV.semimajor_axis               = table2array(T(:, 5)); % meters
REV.eccentricity                 = table2array(T(:, 6)); % unitless
REV.inclination                  = deg2rad(table2array(T(:, 7))); % radians
REV.argument_of_periapsis        = deg2rad(table2array(T(:, 8))); % radians
REV.longitude_of_ascending_node  = deg2rad(table2array(T(:, 9))); % radians
REV.mean_anomaly                 = deg2rad(table2array(T(:, 10))); % radians

%% Convert the ephemerides to orbital elements
num_time = numel(time);

semiparameter_history               = zeros(num_time, 1);
semimajor_axis_history              = zeros(num_time, 1);
eccentricity_history                = zeros(num_time, 1);
inclination_history                 = zeros(num_time, 1);
longitude_of_ascending_node_history = zeros(num_time, 1);
argument_of_periapsis_history       = zeros(num_time, 1);
true_anomaly_history                = zeros(num_time, 1);
argument_of_latitude_history        = zeros(num_time, 1);
true_longitude_history              = zeros(num_time, 1);
true_longitude_of_periapsis_history = zeros(num_time, 1);
eccentric_anomaly_history           = zeros(num_time, 1);
mean_anomaly_history                = zeros(num_time, 1);

for idx = 1:num_time
    [...
        semiparameter, ...
        semimajor_axis, ...
        eccentricity, ...
        inclination, ...
        longitude_of_ascending_node, ...
        argument_of_periapsis, ...
        true_anomaly, ...
        argument_of_latitude, ...
        true_longitude, ...
        true_longitude_of_periapsis] = ...
    OrbitalElementsFromStateVector( ...
        position_history_eci(:, idx), ...
        velocity_history_eci(:, idx), ...
        gravitational_parameter);

    eccentric_anomaly = EccentricAnomalyFromTrueAnomaly(true_anomaly, eccentricity);
    mean_anomaly      = MeanAnomalyFromEccentricAnomaly(eccentric_anomaly, eccentricity);

    semiparameter_history(idx)                  = semiparameter;
    semimajor_axis_history(idx)                 = semimajor_axis;
    eccentricity_history(idx)                   = eccentricity;
    inclination_history(idx)                    = inclination;
    longitude_of_ascending_node_history(idx)    = longitude_of_ascending_node;
    argument_of_periapsis_history(idx)          = argument_of_periapsis;
    true_anomaly_history(idx)                   = true_anomaly;
    argument_of_latitude_history(idx)           = argument_of_latitude;
    true_longitude_history(idx)                 = true_longitude;
    true_longitude_of_periapsis_history(idx)    = true_longitude_of_periapsis;
    eccentric_anomaly_history(idx)              = eccentric_anomaly;
    mean_anomaly_history(idx)                   = mean_anomaly;
end

%% Check that the data is correct
% All correct
% figure(); hold on; plot(OEL.time, OEL.semimajor_axis, "k-");              plot(time, semimajor_axis_history, "r--");
% figure(); hold on; plot(OEL.time, OEL.eccentricity, "k-");                plot(time, eccentricity_history, "r--");
% figure(); hold on; plot(OEL.time, OEL.inclination, "k-");                 plot(time, inclination_history, "r--");
% figure(); hold on; plot(OEL.time, OEL.argument_of_periapsis, "k-");       plot(time, argument_of_periapsis_history, "r--");
% figure(); hold on; plot(OEL.time, OEL.longitude_of_ascending_node, "k-"); plot(time, longitude_of_ascending_node_history, "r--");
% figure(); hold on; plot(OEL.time, OEL.mean_anomaly, "k-");                plot(time, mean_anomaly_history, "r--");

%% Calculate the revolution times
% A revolution is defined as the time between two north-bound crossings of the equator (time when the satellite is at
% the line of nodes). This is when the true_anomaly is equal to 2*pi - argument_of_periapsis. Alternatively, this is
% when the ECI z-position crosses from negative to positive.
% angle_history   = unwrap(2*pi - argument_of_periapsis_history - true_anomaly_history);
% num_crossings = floor(abs(angle_history(end)) / (2*pi));
% crossing_times  = interp1(abs(angle_history), time, 2*pi*(1:1:num_crossings), "pchip");
% crossing_times  = reshape(crossing_times, [], 1);

% This returns this index of the sample immediately before crossing 0.
idxs_immediately_before_crossing = find(diff(sign(position_history_eci(3, :))) > 0);
idxs_immediately_before_crossing = reshape(idxs_immediately_before_crossing, [], 1);
idxs_immediately_after_crossing  = idxs_immediately_before_crossing + 1;

num_crossings  = numel(idxs_immediately_before_crossing);
approx_crossing_times = zeros(num_crossings, 1);

for idx = 1:num_crossings
    % These indices capture the two points before zero crossing and the two points after.
    search_idxs = (-1:1:2) + idxs_immediately_before_crossing(idx);
    % Interpolating with pchip which does a shape-preserving piecewise cubic with 4 neighboring points.
    % In the future, might set up a least squares problem with the position/velocity constraints of the two points
    % surrounding the crossing.
    approx_crossing_times(idx) = interp1( ...
        position_history_eci(3, search_idxs), ...
        time(search_idxs), ...
        0, ...
        "pchip");
end

% Compute the starting and ending indices of each revoluton
mean_revolution_time     = mean(diff(approx_crossing_times));
num_full_num_revolutions = num_crossings - 1;
num_partial_revolutions  = num_full_num_revolutions + 2;

revolution_start_idxs = [1; idxs_immediately_after_crossing];
revolution_end_idxs   = [idxs_immediately_before_crossing; numel(time)];

revolution_start_times = time(revolution_start_idxs);
revolution_end_times = time(revolution_end_idxs);

approx_midpoint_revolution_times = (time(revolution_end_idxs) + time(revolution_start_idxs))/2;

%% Check against provided data
assert(all(abs(revolution_start_times - REV.start_time) < 1e-9, 'all'));
assert(all(abs(revolution_end_times - REV.end_time) < 1e-9, 'all'));

%% Now compute the mean orbital elements
mean_semimajor_axis_history              = zeros(num_partial_revolutions, 1);
mean_eccentricity_history                = zeros(num_partial_revolutions, 1);
mean_inclination_history                 = zeros(num_partial_revolutions, 1);
mean_longitude_of_ascending_node_history = zeros(num_partial_revolutions, 1);
mean_argument_of_periapsis_history       = zeros(num_partial_revolutions, 1);
mean_mean_anomaly_history                = zeros(num_partial_revolutions, 1);

for revolution_idx = 1:num_partial_revolutions
    mean_semimajor_axis_history(revolution_idx)              = mean(semimajor_axis_history(              revolution_start_idxs(revolution_idx):revolution_end_idxs(revolution_idx)));
    mean_eccentricity_history(revolution_idx)                = mean(eccentricity_history(                revolution_start_idxs(revolution_idx):revolution_end_idxs(revolution_idx)));
    mean_inclination_history(revolution_idx)                 = mean(inclination_history(                 revolution_start_idxs(revolution_idx):revolution_end_idxs(revolution_idx)));
    mean_longitude_of_ascending_node_history(revolution_idx) = mean(longitude_of_ascending_node_history( revolution_start_idxs(revolution_idx):revolution_end_idxs(revolution_idx)));
    mean_argument_of_periapsis_history(revolution_idx)       = mean(argument_of_periapsis_history(       revolution_start_idxs(revolution_idx):revolution_end_idxs(revolution_idx)));
    mean_mean_anomaly_history(revolution_idx)                = mean(mean_anomaly_history(                revolution_start_idxs(revolution_idx):revolution_end_idxs(revolution_idx)));
end

%% Plot the mean and osculating elements
% Semimajor axis
figure('Position', [100, 100, 1500, 750]);
hold on;
h1 = plot( ...
    time ./ Units.hours, ...
    semimajor_axis_history ./ Units.kilometers, ...
    "-o", ...
    "LineWidth", 3.0, ...
    "Color", "#D95319");
h2 = plot( ...
    approx_midpoint_revolution_times ./ Units.hours, ...
    mean_semimajor_axis_history ./ Units.kilometers, ...
    "-o", ...
    "LineWidth", 3.0, ...
    "Color", "#0072BD");
grid on;
grid minor;
set(gca, "GridAlpha", 0.75, "MinorGridAlpha", 0.5);
set(gca, "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
xlabel("Time [hr]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
ylabel("a [km]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
title("Semimajor Axis", "FontSize", 18, "FontWeight", "Bold", "FontName", "Times New Roman");
axis tight;
ylim([-0.1, +0.1] .* diff(ylim()) + ylim());
legend([h1, h2], "Osculating", "Mean", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");

% Eccentricity
figure('Position', [100, 100, 1500, 750]);
hold on;
h1 = plot( ...
    time ./ Units.hours, ...
    eccentricity_history, ...
    "-o", ...
    "LineWidth", 3.0, ...
    "Color", "#D95319");
h2 = plot( ...
    approx_midpoint_revolution_times ./ Units.hours, ...
    mean_eccentricity_history, ...
    "-o", ...
    "LineWidth", 3.0, ...
    "Color", "#0072BD");
grid on;
grid minor;
set(gca, "GridAlpha", 0.75, "MinorGridAlpha", 0.5);
set(gca, "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
xlabel("Time [hr]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
ylabel("e [unitless]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
title("Eccentricity", "FontSize", 18, "FontWeight", "Bold", "FontName", "Times New Roman");
axis tight;
ylim([-0.1, +0.1] .* diff(ylim()) + ylim());
legend([h1, h2], "Osculating", "Mean", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");

% Inclination
figure('Position', [100, 100, 1500, 750]);
hold on;
h1 = plot( ...
    time ./ Units.hours, ...
    rad2deg(inclination_history), ...
    "-o", ...
    "LineWidth", 3.0, ...
    "Color", "#D95319");
h2 = plot( ...
    approx_midpoint_revolution_times ./ Units.hours, ...
    rad2deg(mean_inclination_history), ...
    "-o", ...
    "LineWidth", 3.0, ...
    "Color", "#0072BD");
grid on;
grid minor;
set(gca, "GridAlpha", 0.75, "MinorGridAlpha", 0.5);
set(gca, "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
xlabel("Time [hr]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
ylabel("I [deg]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
title("Inclination", "FontSize", 18, "FontWeight", "Bold", "FontName", "Times New Roman");
axis tight;
ylim([-0.1, +0.1] .* diff(ylim()) + ylim());
legend([h1, h2], "Osculating", "Mean", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");

% Argument of periapsis
figure('Position', [100, 100, 1500, 750]);
hold on;
h1 = plot( ...
    time ./ Units.hours, ...
    rad2deg(argument_of_periapsis_history), ...
    "-o", ...
    "LineWidth", 3.0, ...
    "Color", "#D95319");
h2 = plot( ...
    approx_midpoint_revolution_times ./ Units.hours, ...
    rad2deg(mean_argument_of_periapsis_history), ...
    "-o", ...
    "LineWidth", 3.0, ...
    "Color", "#0072BD");
grid on;
grid minor;
set(gca, "GridAlpha", 0.75, "MinorGridAlpha", 0.5);
set(gca, "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
xlabel("Time [hr]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
ylabel("\omega [deg]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
title("Argument of Periapsis", "FontSize", 18, "FontWeight", "Bold", "FontName", "Times New Roman");
axis tight;
ylim([-0.1, +0.1] .* diff(ylim()) + ylim());
legend([h1, h2], "Osculating", "Mean", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");

% Longitude of the ascending node
figure('Position', [100, 100, 1500, 750]);
hold on;
h1 = plot( ...
    time ./ Units.hours, ...
    rad2deg(longitude_of_ascending_node_history), ...
    "-o", ...
    "LineWidth", 3.0, ...
    "Color", "#D95319");
h2 = plot( ...
    approx_midpoint_revolution_times ./ Units.hours, ...
    rad2deg(mean_longitude_of_ascending_node_history), ...
    "-o", ...
    "LineWidth", 3.0, ...
    "Color", "#0072BD");
grid on;
grid minor;
set(gca, "GridAlpha", 0.75, "MinorGridAlpha", 0.5);
set(gca, "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
xlabel("Time [hr]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
ylabel("\Omega [deg]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
title("Longitude of the Ascending Node", "FontSize", 18, "FontWeight", "Bold", "FontName", "Times New Roman");
axis tight;
ylim([-0.1, +0.1] .* diff(ylim()) + ylim());
legend([h1, h2], "Osculating", "Mean", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");

% Mean Anomaly
figure('Position', [100, 100, 1500, 750]);
hold on;
h1 = plot( ...
    time ./ Units.hours, ...
    rad2deg(mean_anomaly_history), ...
    "-o", ...
    "LineWidth", 3.0, ...
    "Color", "#D95319");
h2 = plot( ...
    approx_midpoint_revolution_times ./ Units.hours, ...
    rad2deg(mean_mean_anomaly_history), ...
    "-o", ...
    "LineWidth", 3.0, ...
    "Color", "#0072BD");
grid on;
grid minor;
set(gca, "GridAlpha", 0.75, "MinorGridAlpha", 0.5);
set(gca, "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
xlabel("Time [hr]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
ylabel("M [deg]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
title("Mean Anomaly", "FontSize", 18, "FontWeight", "Bold", "FontName", "Times New Roman");
axis tight;
ylim([-0.1, +0.1] .* diff(ylim()) + ylim());
legend([h1, h2], "Osculating", "Mean", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");

%% Principle periodicity analysis
revolution_time = time / mean_revolution_time;

% Semimajor axis
[f, P] = SpectralDecomposition(revolution_time, semimajor_axis_history);

figure('Position', [100, 100, 600, 400]);
hold on;
plot(f, P, ...
    'k-', "LineWidth", 3.0);
grid on;
grid minor;
set(gca, "GridAlpha", 0.75, "MinorGridAlpha", 0.5);
set(gca, "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
xlabel("Frequency per Orbit [unitless]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
ylabel("Power [ m^2 ]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
title("Power Spectrum of Semimajor Axis", "FontSize", 18, "FontWeight", "Bold", "FontName", "Times New Roman");
axis tight;
yscale('log');
xlim([0, 8])
ylim([1/10, 10] .* ylim());

% Eccentricity
[f, P] = SpectralDecomposition(revolution_time, eccentricity_history);

figure('Position', [100, 100, 600, 400]);
hold on;
plot(f, P, ...
    'k-', "LineWidth", 3.0);
grid on;
grid minor;
set(gca, "GridAlpha", 0.75, "MinorGridAlpha", 0.5);
set(gca, "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
xlabel("Frequency per Orbit [unitless]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
ylabel("Power [ unitless ]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
title("Power Spectrum of Eccentricity", "FontSize", 18, "FontWeight", "Bold", "FontName", "Times New Roman");
axis tight;
yscale('log');
xlim([0, 8])
ylim([1/10, 10] .* ylim());

% Inclination
[f, P] = SpectralDecomposition(revolution_time, rad2deg(inclination_history));

figure('Position', [100, 100, 600, 400]);
hold on;
plot(f, P, ...
    'k-', "LineWidth", 3.0);
grid on;
grid minor;
set(gca, "GridAlpha", 0.75, "MinorGridAlpha", 0.5);
set(gca, "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
xlabel("Frequency per Orbit [unitless]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
ylabel("Power [ deg^2 ]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
title("Power Spectrum of Inclination", "FontSize", 18, "FontWeight", "Bold", "FontName", "Times New Roman");
axis tight;
yscale('log');
xlim([0, 8])
ylim([1/10, 10] .* ylim());

% Argument of periapsis
[f, P] = SpectralDecomposition(revolution_time, rad2deg(argument_of_periapsis_history));

figure('Position', [100, 100, 600, 400]);
hold on;
plot(f, P, ...
    'k-', "LineWidth", 3.0);
grid on;
grid minor;
set(gca, "GridAlpha", 0.75, "MinorGridAlpha", 0.5);
set(gca, "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
xlabel("Frequency per Orbit [unitless]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
ylabel("Power [ deg^2 ]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
title("Power Spectrum of Argument of Periapsis", "FontSize", 18, "FontWeight", "Bold", "FontName", "Times New Roman");
axis tight;
yscale('log');
xlim([0, 8])
ylim([1/10, 10] .* ylim());

% Argument of periapsis
[f, P] = SpectralDecomposition(revolution_time, rad2deg(longitude_of_ascending_node_history));

figure('Position', [100, 100, 600, 400]);
hold on;
plot(f, P, ...
    'k-', "LineWidth", 3.0);
grid on;
grid minor;
set(gca, "GridAlpha", 0.75, "MinorGridAlpha", 0.5);
set(gca, "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
xlabel("Frequency per Orbit [unitless]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
ylabel("Power [ deg^2 ]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
title("Power Spectrum of Longitude of Ascending Node", "FontSize", 18, "FontWeight", "Bold", "FontName", "Times New Roman");
axis tight;
yscale('log');
xlim([0, 8])
ylim([1/10, 10] .* ylim());

% Argument of mean anomaly
[f, P] = SpectralDecomposition(revolution_time, rad2deg(mean_anomaly_history));

figure('Position', [100, 100, 600, 400]);
hold on;
plot(f, P, ...
    'k-', "LineWidth", 3.0);
grid on;
grid minor;
set(gca, "GridAlpha", 0.75, "MinorGridAlpha", 0.5);
set(gca, "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
xlabel("Frequency per Orbit [unitless]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
ylabel("Power [ deg^2 ]", "FontSize", 14, "FontWeight", "Bold", "FontName", "Times New Roman");
title("Power Spectrum of Mean Anomaly", "FontSize", 18, "FontWeight", "Bold", "FontName", "Times New Roman");
axis tight;
yscale('log');
xlim([0, 8])
ylim([1/10, 10] .* ylim());


%% Helper functions
function [frequencies, data_power] = SpectralDecomposition( ...
        time, ...
        data)

    % Metadata
    num_samples      = numel(data);
    sample_frequency = 1.0 / (time(2) - time(1));                               
    frequencies      = ((-num_samples/2):(num_samples/2 - 1)) * (sample_frequency/num_samples);

    % Remove the mean from the data
    mean_removed_data = data - mean(data);

    % Compute the power per frequency in the data
    data_fft   = fft(mean_removed_data);
    data_power = abs(data_fft).^2 / num_samples;
    data_power = fftshift(data_power);

    % Remove the mean component from reporting
    [~, min_idx] = min(data_power);
    data_power(min_idx) = [];
    frequencies(min_idx) = [];

    % Remove negative frequencies
    mask = frequencies > 0;
    data_power = data_power(mask);
    frequencies = frequencies(mask);
end

