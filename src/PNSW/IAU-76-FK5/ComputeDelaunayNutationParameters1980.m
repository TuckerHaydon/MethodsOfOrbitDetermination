function [ ...
        ...
        ComputeDelaunayNutationParameters1980(julian_centuries_since_epoch)
    % Computes the Delaunay nutation parameters using the IAU 1980 nutation model.
    % 
    % Requires:
    % - julian_centuries_since_epoch:
    %   - (:, :) double.
    %   - The number of julian centuries since the J2000 epoch.
    %
    % Returns:
    % - moon_mean_anomaly_deg:
    %  - (:, :) double.
    ...
    % Vallado 3-85
    % Delaunay Parameters
    M_moon_deg               = 134.96298139 + (1325 .* r  + 198.8673981) .* TTT + 0.0086972 .* TTT.^2 + 1.78e-5 .* TTT.^3;
    M_sun_deg                = 357.52772333 + (99   .* r  + 359.0503400) .* TTT - 0.0001603 .* TTT.^2 - 3.3e-6  .* TTT.^3;
    u_bar_moon_deg           = 93.27191028  + (1342 .* r  + 82.0175381)  .* TTT - 0.0036825 .* TTT.^2 + 3.1e-6  .* TTT.^3;
    D_sun_deg                = 297.85036306 + (1236 .* r  + 307.1114800) .* TTT - 0.0019142 .* TTT.^2 + 5.3e-6  .* TTT.^3;
    lambda_bar_ecliptic_deg  = 125.04452222 - (5    .* r  + 134.1362608) .* TTT + 0.0020708 .* TTT.^2 + 2.2e-6  .* TTT.^3;
