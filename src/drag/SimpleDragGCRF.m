function [drag_gcrf] = SimpleDragGCRF(...
    ...
    arguments(Input)
        ...
        effective_area(:, 1) double {mustBeReal, mustBeFinite}
    ...
    assert(numel(effective_area) == num_vectors);
    ...
    % Compute the drag
    % Size [3, N]
    drag_gcrf = -1/2 .* transpose(C_d) .* (transpose(A_eff) ./ m) .* transpose(rho_A) .* transpose(V_A_mag) .* V_A_vec;
end