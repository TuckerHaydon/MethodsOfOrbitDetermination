function [moon_position_tod] = ComputeMoonPositionTOD( ...
        tdb_time, ...
        earth_radius)
    % Computes the moon position vector in the true of date (TOD) frame.
    %
    % Requires:
    % - tdb_time:
    %   - (1, 1) datetime.
    ...
    % - moon_position_tod:
    %   - (3, 1) double.
    %   - The moon position vector in the TOD frame.
    %   - Unit meters.
    % 
    % References:
    ...
    arguments(Output)
        moon_position_tod(3, 1) double {mustBeReal, mustBeFinite}
    end
